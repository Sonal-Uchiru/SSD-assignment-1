    ...
    const updateArea = () => {
      ...
      if (data.features.length > 0) {
        if (data.features.length == 1) {
          const fieldRectagularShape = getRectangularShape(data.features[0]);
  ...
  const savePaddyFieldAsync = async () => {
    ...
    try {
      ...
      const data = {
        ...
        spaceBetweenIrrigations: response.data.metaData.spaceBetweenIrrigations,
      };
  ...
  return (
    <Box sx={{ minHeight: 650 }}>
      ...
      {successModalVisibility && (
        <SuccessModal handleCancel={() => setSuccessModalVisibility(false)} />
      )}
    </Box>
  );
}
